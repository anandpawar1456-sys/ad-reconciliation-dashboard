...
type Point = { date: string; ghlRevenue: number; metaRevenue: number };
...
export default function RevenueChart({ points }: { points: Point[] }) {
  ...
  const maxValue = useMemo(() => {
    const max = Math.max(1, ...points.map((p) => Math.max(p.ghlRevenue, p.metaRevenue)));
    return max * 1.15;
  }, [points]);
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {points.map((p, i) =>
          i % labelStep === 0 ? (
            <text key={i} x={x(i)} y={HEIGHT - 8} textAnchor="middle" className="fill-ink-400 text-[9px]">
              {formatLabelDate(new Date(p.date)).replace(",", "").replace(/\d{4}$/, "")}
            </text>
          ) : null
        )}
      ...
      {hoverIndex !== null && (
        <div
          className="pointer-events-none absolute top-2 rounded-xl border border-white/60 bg-white/95 px-3 py-2 text-xs shadow-soft backdrop-blur"
          style={{
            left: `min(${(x(hoverIndex) / WIDTH) * 100}%, calc(100% - 150px))`,
          }}
        >
          <div className="font-semibold text-ink-900">{formatLabelDate(new Date(points[hoverIndex].date))}</div>
